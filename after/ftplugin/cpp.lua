...
function _MAKE_TEST()
	return Terminal:new({ cmd = "make test", float_opts = { border = "rounded", }, direction = "float", hidden = false, close_on_exit = false })
	    :toggle()
end

vim.keymap.set("n", "<F3>", ":lua _MAKE()<cr>", opts)
vim.keymap.set("n", "<F4>", ":lua _MAKE_RUN()<cr>", opts)
vim.keymap.set("n", "<F5>", ":lua _MAKE_TEST()<cr>", opts)
